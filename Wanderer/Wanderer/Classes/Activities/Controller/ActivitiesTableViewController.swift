...
extension ActivitiesTableViewController {
    ...
    func reuqestData(page : Int) {
        ...
        NetWorkingTools.sharedInstance.requestAllAcitivities(page) { (result, error) in
            guard let resultDictArray = result else {
                SVProgressHUD.showErrorWithStatus("网络数据出错")
                return
            }
            for dict in resultDictArray {
                if let dictAcitivity = dict["activity"] as? [String : AnyObject]{
                    let model =  DestinationModel(dict: dictAcitivity)
                    model.isFull = true
